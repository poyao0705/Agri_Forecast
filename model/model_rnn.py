...
class SRNN_VE1(nn.Module):
    def __init__(self, input_dim=1, hidden_dim=1):
        super(SRNN_VE1, self).__init__()
        self.rnn = nn.RNN(
    ...
    def forward(self, x, h_prev=None):
        out, h_n = self.rnn(x, h_prev)  # h_n: [1, batch, hidden_dim]
        out_last = out[:, -1, :]  # use last time step
        raw_output = self.output_layer(out_last)  # ✅ first get raw output

        raw_output = torch.clamp(raw_output, -10, 10)  # ✅ then clamp

        scale = 0.05  # Increased scale for better VaR calibration
        # VaR should be negative (representing loss)
        var = -scale * torch.exp(torch.clamp(raw_output[:, 0], -3, 3))
        # ES should be more negative than VaR (average loss beyond VaR)
        es = var - scale * torch.exp(torch.clamp(raw_output[:, 1], -3, 3))

        return torch.stack([var, es], dim=1), h_n
...
class BasicFZ0Loss(nn.Module):
    ...
    def forward(self, y_true, y_pred):
        ...
        # FZ0 loss: (1/α) * E[I(Y ≤ VaR) * (VaR - Y)] + E[VaR/ES + log(-ES) - 1]
...
def train_with_overlapping_evaluate_with_expanding(
    ...
    model = SRNN_VE1(input_dim=input_dim, hidden_dim=8)
    ...
    torch.save(model.state_dict(), "model/saved_models/var_es_rnn_hybrid.pth")
...
def evaluate_frozen_model_with_expanding_window(
    ...
    with torch.no_grad():
        ...
        for i in range(test_start_idx, len(X)):
            ...
            prediction, h_state = model(X_tensor, h_state)
...
def hybrid_approach_main():
    ...
    joblib.dump(scaler, "model/saved_models/scaler_rnn.pkl")
    ...
    print("=" * 60)
    print("RNN Hybrid Approach Evaluation")
    print("=" * 60)
    ...
    plt.legend()
    plt.title("RNN Hybrid Approach: VaR Backtesting")
