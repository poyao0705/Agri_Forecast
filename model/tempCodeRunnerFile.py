...
class ImprovedVaRTransformer(nn.Module):
    """
    ...
    def forward(self, x):
        ...
        # Better VaR/ES formulation - use tanh to bound the output
        var_raw = torch.tanh(out[:, 0])  # Bounded between -1 and 1
        es_raw = torch.tanh(out[:, 1])  # Bounded between -1 and 1

        # Scale VaR to reasonable range (e.g., -0.1 to 0.1)
        var = -0.05 * var_raw  # VaR is negative, scaled to reasonable range

        # ES should be more negative than VaR
        es_offset = 0.02 * es_raw + 0.01  # Small offset
        es = var - es_offset

        return torch.stack([var, es], dim=1)


class ImprovedFZ0Loss(nn.Module):
    """
    ...
    def forward(self, y_true, y_pred):
        ...
        # Stronger hit rate penalty to target exactly alpha
        hit_rate = torch.mean(indicator)
        target_hit_rate = self.alpha  # Target exactly 5%
        hit_rate_penalty = (
            torch.abs(hit_rate - target_hit_rate) * 20.0  # Much stronger penalty
        )

        # Stronger penalty for overly conservative predictions
        conservative_penalty = (
            torch.mean(torch.relu(-var - 0.03)) * 10.0
        )  # Stronger penalty
        ...
        total_loss = (
            ...
            + hit_rate_penalty
            + conservative_penalty  # Stronger penalty
...
def train_with_overlapping_evaluate_with_expanding(
    X, y, input_dim, max_epochs=300, batch_size=64, patience=50, seq_len=21, overlap=0.3
):
    """
    ...
    loss_fn = ImprovedFZ0Loss(alpha=0.05, lambda_reg=0.05, es_penalty_weight=1.5)

    optimizer = torch.optim.AdamW(
        model.parameters(), lr=1e-4, weight_decay=1e-5
    )  # Lower learning rate
    scheduler = StepLR(optimizer, step_size=50, gamma=0.9)  # Slower decay
    ...
    for epoch in range(max_epochs):
        ...
        if epoch % 10 == 0:
            # Calculate hit rate for monitoring
            model.eval()
            with torch.no_grad():
                train_hits = 0
                total_train = 0
                for X_batch, y_batch in train_loader:
                    y_pred = model(X_batch)
                    var_pred = y_pred[:, 0]
                    hits = (y_batch <= var_pred).float()
                    train_hits += hits.sum().item()
                    total_train += len(y_batch)

                train_hit_rate = train_hits / total_train if total_train > 0 else 0

                val_hits = 0
                total_val = 0
                for X_val, y_val in val_loader:
                    y_val_pred = model(X_val)
                    var_val_pred = y_val_pred[:, 0]
                    hits = (y_val <= var_val_pred).float()
                    val_hits += hits.sum().item()
                    total_val += len(y_val)

                val_hit_rate = val_hits / total_val if total_val > 0 else 0

            print(
                f"Epoch {epoch+1}, Train Loss: {avg_train_loss:.6f}, Val Loss: {avg_val_loss:.6f}"
            )
            print(
                f"Train Hit Rate: {train_hit_rate:.4f}, Val Hit Rate: {val_hit_rate:.4f} (Target: 0.05)"
            )
            model.train()
...
def train_with_overlapping_evaluate_with_expanding(
    ...
    print(f"Training with {len(X_seq)} overlapping sequences...")
    model = ImprovedVaRTransformer(input_dim=input_dim)
    loss_fn = ImprovedFZ0Loss(alpha=0.05, lambda_reg=0.1, es_penalty_weight=2.0)
